...
def test_l2_distance_dsl_matches_python() -> None:
    ...
    torch.manual_seed(0)
    indices = [0, 1, 2, 3, 4]
    traj = dist.gm.mean().detach().double().unsqueeze(0).repeat(
        8, 1
    ) + 0.05 * torch.randn(8, n, dtype=torch.float64)

    py = l2_distance(traj, indices)(dist)
    dsl = compile_loss(
        LOSS_DSL_SOURCE["l2_distance"], trajectories=traj, indices=indices
    )(dist)
...
def test_neg_log_likelihood_dsl_matches_python() -> None:
    ...
    dsl = compile_loss(
        LOSS_DSL_SOURCE["neg_log_likelihood"], trajectories=traj, indices=indices
    )(dist)
