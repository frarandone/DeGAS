"""Merge and pruning rules for SOGA branch distributions."""
...
def merge(list_dist: list[tuple[torch.Tensor, Dist]]) -> tuple[torch.Tensor, Dist]:
    """
    ...
    # if list is empty
    if len(p_list) == 0:
        return torch.tensor(0.0), list_dist[0][1]
    # else
...
def prune(current_dist: Dist, pruning: str, Kmax: int) -> Dist:
    if pruning == "classic":
...
def kmeans_prune(output_dist: Dist, Kmax: int) -> Dist:
    ...
    new_pis.scatter_add_(0, labels.view(-1, 1), output_dist.gm.pi)
    normalized_pis = output_dist.gm.pi / new_pis[labels]

    # computes the new means
    weighted_mus = output_dist.gm.mu * normalized_pis
    new_mus = torch.zeros(Kmax, d)
    new_mus.scatter_add_(0, labels.view(-1, 1).expand(-1, d), weighted_mus)

    # computes the new covariances
    diff = output_dist.gm.mu - new_mus[labels]
    weighted_sigmas = normalized_pis.view(-1, 1, 1) * output_dist.gm.sigma + torch.einsum(
        "bi,bj->bij", diff, diff
    ) * normalized_pis.view(-1, 1, 1)
...
def k_means(points: torch.Tensor, k: int, max_iters: int = 100, tol: float = 1e-4) -> tuple[torch.Tensor, torch.Tensor]:
...
def ranking_prune(current_dist: Dist, Kmax: int) -> Dist:
...
def compute_matrix_mean(current_dist: Dist) -> torch.Tensor:
...
def delete_indices(tensor: torch.Tensor, idx_list: list[int]) -> torch.Tensor:
...
def merge_comp(current_dist: Dist, i: int, j: int, tot_mean: torch.Tensor) -> Dist:
...
def classic_prune(current_dist: Dist, Kmax: int) -> Dist:
    ...
    if current_dist.gm.n_comp() > Kmax:
        ...
        dist_matrix = torch.sum(torch.pow((current_dist.gm.mu - matrix_mu), 2), axis=2).T
        weight_dist_matrix = current_dist.gm.pi.view(-1, 1) * dist_matrix
        cost_matrix = weight_dist_matrix + weight_dist_matrix.T
        ...
        while n > Kmax:
            ...
            if n > Kmax:
                ...
                new_cost = torch.sum(torch.pow((current_dist.gm.mu[-1].unsqueeze(0) - matrix_mu[:, -1]), 2), axis=1)
                new_cost = new_cost.unsqueeze(0)
