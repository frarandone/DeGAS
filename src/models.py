...
def thermostat(T=10, init_T=17., tOn=18, tOff=20, k=0.01, h=0.5, eps=0.1, device='cuda:0'):
    traj = torch.zeros(T, device=device)
    isOn = False
    traj[0] = init_T
    noise = distributions.Normal(torch.tensor(0., device=device), torch.tensor(eps, device=device))
...
def gearbox(T=20, init_v=5., gear=1, s1=10, s2=20, device='cuda:0'):
    ...
    for i in range(1,T):
        ...
        #update velocity
        if gear >= 1:
            traj[i] = traj[i-1] + 0.1*traj[i-1]*alpha[gear-1] + 0.1*distributions.Normal(torch.tensor(5., device=device), torch.tensor(1., device=device)).rsample() 
        else:
            traj[i] = traj[i-1] - 0.1*0.0005*traj[i-1]**2 + distributions.Normal(torch.tensor(0., device=device), torch.tensor(1., device=device)).rsample() 
...
def pid(T=50, init_ang = 0.5, b1=-1, b2=-2, b3=1, b4=-6, s0=1, s1=1, s2=1, device='cuda:0'):
    ...
    noise = distributions.Normal(torch.tensor(0., device=device), torch.tensor(0.25, device=device))
    noise_ang = distributions.Normal(torch.tensor(0., device=device), torch.tensor(0.25, device=device))
