...
def batch_means_conf_interval(data, batch_size, confidence_level=0.05):
    ...
    margin_of_error = (
        np.std(batch_means, axis=0)
        * np.sqrt(1 - confidence_level)
        / np.sqrt(batch_means.shape[0])
    )
    err = (margin_of_error / np.mean(batch_means, axis=0)) * 100

    return (
        np.mean(batch_means, axis=0),
        batch_means.mean(axis=0) - margin_of_error,
        batch_means.mean(axis=0) + margin_of_error,
        err,
    )
...
if __name__ == "__main__":
    ...
    batch_means_result, lw, up, err = batch_means_conf_interval(
        data, batch_size, confidence_level
    )
    print(err)
