...
def smooth_asgmt_gm(var, expr, eps):
    ...
    stds = stds.strip("[]").split(
        ","
    )  # the problem is that one ore more stds could be a parameter
    for i, std in enumerate(stds):
        if "_" not in std and eval(std) == 0:
...
def update_smoothed_vars(smoothed_vars, var_name, idx, var_list, data):
    ...
    if idx is None:  # var_name is a variable
        ...
        idx in data.keys()
    ):  # var_name is of type var_name[index_name] all variable var_name are smoothed
        for var in var_list:
...
def smooth_asgmt(node, var_list, smoothed_vars, data, smooth_eps):
    ...
    if len(vars) == 0:
        ...
        smoothed_vars = update_smoothed_vars(
            smoothed_vars, var_name, idx, var_list, data
        )
    ...
    if len(vars) > 0 and len(gm_vars) > 0:
        for gm in gm_vars:  # only smooths if gm is degenerate
            ...
            stds = stds.strip("[]").split(",")
            for std in stds:
                if "_" not in std and eval(std) == 0:  # stds can be parameters
    ...
    if len(vars) > 0 and len(gm_vars) == 0:
        if (
            not expr_is_prod(orig_expr, vars) and target_var not in vars
        ):  # if the expression is not a product of two variables and the target variable is not in the expression
            new_orig_expr = orig_expr + "+ gm([1.], [0.], [{:.10f}])".format(smooth_eps)
...
def smooth_trunc(trunc, node, smoothed_vars, smooth_eps):
    ...
    if target_var.replace(" ", "") not in smoothed_vars:
        return trunc

    # currently selects delta as a constant function of epsilon. TO DO: make it a function of the distribution.
    delta = 5 * np.sqrt(
        smooth_eps
    )  # maybe this should not be linear in smooth_eps (see Continualization)

    if ops == "==":
        new_trunc = "{} > {} - {:.10f} and {} < {} + {:.10f}".format(
            target_var, target_val, delta, target_var, target_val, delta
        )
    elif ops == "!=":
        new_trunc = "{} < {} - {:.10f} or {} > {} + {:.10f}".format(
            target_var, target_val, delta, target_var, target_val, delta
        )
    elif ops == "<=":
...
def smooth_cfg(
    cfg, params_dict={}, pruning=None, Kmax=None, parallel=None, smooth_eps=1e-3
):
...
def smoother(
    node, var_list, smoothed_vars, data, parallel, exec_queue, params_dict, smooth_eps
):
    ...
    if node.type == "state":
        if node.cond != None and node.trunc is not None:
            if node.cond == False:
