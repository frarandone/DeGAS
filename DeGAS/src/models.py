...
def thermostat(T=10, init_T=17.0, tOn=18, tOff=20, k=0.01, h=0.5, eps=0.1):
    traj = torch.zeros(T)
    isOn = False
    traj[0] = init_T
...
def gearbox(T=20, init_v=5.0, gear=1, s1=10, s2=20):
    ...
    for i in range(1, T):
        ...
        if gear >= 1:
            traj[i] = (
                traj[i - 1]
                + 0.1 * traj[i - 1] * alpha[gear - 1]
                + 0.1 * distributions.Normal(5.0, 1.0).rsample()
            )
        else:
            traj[i] = (
                traj[i - 1]
                - 0.1 * 0.0005 * traj[i - 1] ** 2
