    ...
    if name == "bouncing_ball":
        loss = lambda dist: -(
            ...
            - dist.gm.marg_cdf(
                    ...
                    dist.gm.mean()[
                        int(
                            dist.gm.mean()[
                                [
                                    i
                                    for i, var in enumerate(dist.var_list)
                                    if var == "valid"
                                ][0]
                            ]
                        ) : 35
                ...
                + int(
                    dist.gm.mean()[
                        [i for i, var in enumerate(dist.var_list) if var == "valid"][0]
                    ]
                ),
            )
        )
        ...
        loss = lambda dist: dist.gm.marg_cdf(torch.tensor([16.0]), idx=indexes)
        ...
        loss = lambda dist: -(
            ...
            * dist.gm.marg_pdf(
                ...
                idx=[
                    int(np.round(0.6 / dt)) + 25,
                    int(np.round(1.8 / dt)) + 25,
                    int(np.round(2.4 / dt)) + 25,
                ],
            ).squeeze()
        )
...
def process(name, all=False):
    ...
    for i in range(t):
        y_init.append(
            output_dist.gm.mean()[
                output_dist.var_list.index(traj_var + "[" + str(i) + "]")
            ]
            .detach()
            .numpy()
        )
