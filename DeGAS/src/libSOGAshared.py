...
class GaussianMix:
    ...
    def __init__(self, pi, mu, sigma):
        ...
        self.sigma = (
            sigma  # sigma is a tensor (c, d, d) where d is the dimension of the space
        )
    ...
    def comp_pdf(self, x, k):
        if self.n_dim() > 1:
            try:
                return torch.exp(
                    distributions.MultivariateNormal(
                        self.mu[k], covariance_matrix=self.sigma[k]
                    ).log_prob(x)
                )
            except ValueError:
                sigma = self.sigma[k]
            ...
            return torch.exp(
                distributions.Normal(self.mu[k], torch.sqrt(self.sigma[k])).log_prob(x)
            ).reshape(x.shape)

    def marg_comp_pdf(self, x, k, idx):
        if isinstance(idx, list):
            cov_submatrix = torch.clone(
                self.sigma[k][torch.tensor(idx).unsqueeze(1), torch.tensor(idx)]
            )
            try:
                return torch.exp(
                    distributions.MultivariateNormal(
                        self.mu[k][idx], cov_submatrix
                    ).log_prob(x)
                )
            except ValueError:
                ...
                if not is_sym:
                    ...
                    self.sigma[k][torch.tensor(idx).unsqueeze(1), torch.tensor(idx)] = (
                        new_cov_submatrix
                    ) = make_sym(cov_submatrix)
                return torch.exp(
                    distributions.MultivariateNormal(
                        self.mu[k][idx], covariance_matrix=new_cov_submatrix
                    ).log_prob(x)
                )
        else:
            return torch.exp(
                distributions.Normal(
                    self.mu[k][idx], torch.sqrt(self.sigma[k][idx, idx])
                ).log_prob(x)
            )

    def pdf(self, x):
        comp_pdfs = torch.stack(
            [self.comp_pdf(x, k) for k in range(self.n_comp())], dim=1
        )
    ...
    def marg_pdf(self, x, idx):
        comp_pdfs = torch.stack(
            [self.marg_comp_pdf(x, k, idx) for k in range(self.n_comp())], dim=1
        )
    ...
    def cdf(self, x):
        comp_cdfs = torch.stack(
            [self.comp_cdf(x, k) for k in range(self.n_comp())], dim=1
        )
    ...
    def marg_cdf(self, x, idx):
        comp_cdfs = torch.stack(
            [self.marg_comp_cdf(x, k, idx) for k in range(self.n_comp())], dim=1
        )
        cdf = torch.matmul(comp_cdfs, self.pi.view(-1, 1))
        return cdf
...
def mvncdf(x, mean, cov):
    ...
    bounds = torch.stack(
        [
            torch.tensor([[-torch.inf, x[i, j] - mean[j]] for j in range(dim)])
    ...
    for i in range(batch_size):
        ...
        if result.isnan():
            cov = make_sym(cov)
            result = torch.exp(
                mvn.MVNXPB(covariance_matrix=cov, bounds=bounds[i]).solve()
            )
        res[i] = result
    return res
...
class TruncatedNormal:
    ...
    def __init__(self, loc, scale, a, b):
        ...
        # auxiliary normal
        self.norm = distributions.Normal(
    ...
    def mean(self):
        return (
            self.loc
            + self.scale.squeeze(2) * (self.phi_alpha - self.phi_beta) / self.norm_const
        )
    ...
    def var(self):
        ...
        return (
            ...
            * (
                torch.tensor(1.0)
                - (prod_beta - prod_alpha) / self.norm_const
                - ((self.phi_alpha - self.phi_beta) / self.norm_const) ** 2
            )
        ).unsqueeze(2)
...
def extend_dist(self, dist):
    ...
    if len(self.aux_pis) > 0:
        ...
        for part in product(*[range(len(mean)) for mean in self.aux_means]):
            ...
            aux_pi = torch.prod(
                torch.stack([self.aux_pis[i][part[i]] for i in range(len(part))])
            )
            new_pis = torch.vstack([new_pis, (aux_pi * dist.gm.pi)])
            # for each combination creates new means
            aux_mu = torch.hstack(
                [self.aux_means[i][part[i]] for i in range(len(part))]
            )
            new_mus = torch.vstack(
                [
                    new_mus,
                    torch.cat(
                        [dist.gm.mu, aux_mu.expand(dist.gm.n_comp(), len(aux_mu))],
                        dim=1,
                    ),
                ]
            )
            # for each combination creates new covs
            aux_sigma = torch.diag(
                torch.hstack([self.aux_covs[i][part[i]] for i in range(len(part))])
            )
