...
def create_thermostat_pyro(initial_params, eps=1.0):
    ...
    def thermostat_pyro(observed_traj, eps=eps, k=0.01, h=0.5):
        ...
        for i in range(1, T):
            traj[:, i] = (
                traj[:, i - 1] - k * traj[:, i - 1] + noise.sample([batch_size])
            )
            traj[:, i] += h * isOn.float()
            isOn = torch.where(
                (isOn & (traj[:, i] > tOff)) | (~isOn & (traj[:, i] < tOn)), ~isOn, isOn
            )
...
def create_thermostat_smooth_pyro(initial_params, sharpness, eps=1.0):
    ...
    def thermostat_smooth_pyro(
        observed_traj, sharpness=sharpness, eps=eps, k=0.01, h=0.5
    ):
        ...
        for i in range(1, T):
            ...
            for b in range(batch_size):
                ...
                if gear[b] >= 1:
                    traj[b, i] = (
                        traj[b, i - 1]
                        + dt * traj[b, i - 1] * alpha[gear[b] - 1]
                        + dt * distributions.Normal(5.0, 1.0).rsample()
                    )
                else:
                    traj[b, i] = (
                        traj[b, i - 1]
                        - dt * 0.0005 * traj[b, i - 1] ** 2
...
def run_NUTS(
    ...
    nuts_kernel = NUTS(
        pyro_model, adapt_step_size=adapt_step_size, step_size=step_size
    )  # NUTS fails to converge (acc. prob = 0.0 both with nonsmooth and smooth)
    mcmc = MCMC(
        nuts_kernel,
...
def run_SVI(
    model, guide, traj_set, num_steps=1000, lr=0.1, num_particles=1, loss_plot=False
):
    pyro.clear_param_store()
    optimizer = pyro.optim.Adam({"lr": lr})
    svi = SVI(
        model, guide, optimizer, loss=Trace_ELBO(num_particles=num_particles)
    )  # with a 100 num_particles and the smooth model the training is more stable
