...
def process(name, all=False):
    ...
    compiledFile = compile2SOGA(
        "../../programs/SOGA/Optimization/CaseStudies/" + name + ".soga"
    )
    ...
    # loss
    print(
        "Initial loss value for ",
        name,
        " : ",
        neg_log_likelihood(orig_traj, output_dist, idx),
    )
    ...
    for i in range(T):
        y_init.append(
            output_dist.gm.mean()[
                output_dist.var_list.index(var_name + "[" + str(i) + "]")
            ]
            .detach()
            .numpy()
        )
    ...
    loss_list, time_opt, n_iters = optimize(
        cfg, params_dict, loss, n_steps=n_steps, lr=lr, print_progress=False
    )
    ...
    loss_list = (np.array(loss_list) - np.min(loss_list)) / (
    ...
    if all:
        ...
        plt.fill_between(
            range(T),
            np.array(y_init) - np.array(y_var_init),
            np.array(y_init) + np.array(y_var_init),
            alpha=0.2,
        )
