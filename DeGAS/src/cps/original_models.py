...
def thermostat_model(T=20, init_T=15.0, tOn=16, tOff=20, k=0.01, h=0.5, eps=0.1):
    T = int(T)
    traj = torch.zeros(T)
    isOn = False
    traj[0] = init_T
    noise = distributions.Normal(torch.tensor(0.0), torch.tensor(eps))
...
def gearbox(T=20, init_v=5.0, gear=1, s1=10, s2=20):
    traj = torch.zeros(T)
    traj[0] = init_v
    ...
    for i in range(1, T):
        ...
        if gear >= 1:
            traj[i] = (
                traj[i - 1]
                + dt * traj[i - 1] * alpha[gear - 1]
                + dt * distributions.Normal(5.0, 1.0).rsample()
            )
        else:
            traj[i] = (
                traj[i - 1]
                - dt * 0.0005 * traj[i - 1] ** 2
                + distributions.Normal(0.0, 1.0).rsample()
            )
...
def pid(T=50, init_ang=0.5, s0=1, s1=1, s2=1):
    ...
    noise = distributions.Normal(torch.tensor(0.0), torch.tensor(0.25))
    noise_ang = distributions.Normal(torch.tensor(0.0), torch.tensor(0.25))
...
def bouncing_ball(T=30, init_H=9.0, eps=0.1, R=5.0, C=400, m=7.0):
    g = 9.81
    # R = 5.0
...
def get_orig_params(name):
    if name == "thermostat":
        orig_params = {
            "T": 30,  # time steps
            "init_T": 16.0,  # initial temperature
            "tOn": 17,  # turn-on temperature
            "tOff": 20,
        }
        n_traj = 100
        params = {"tOn": 15.0, "tOff": 22.0}
        var_name = "T"
        ...
        orig_params = {
            "T": 20,  # time steps
            "init_v": 5.0,  # initial velocity
            "gear": 1,  # initial gear
            "s1": 10.0,
            "s2": 20.0,
        }
        n_traj = 1000
        params = {"s1": 8.0, "s2": 12.0}
        var_name = "v"
        model = gearbox
        n_steps = 200
        lr = 0.3
        ...
        orig_params = {
            "T": 35,  # time steps
            "init_H": 9.0,  # initial height
            "R": 7.0,
            "C": 400.0,
            "m": 7.0,
        }
        n_traj = 100
        params = {"R": -1.0, "C": 450.0}
        var_name = "H"
        model = bouncing_ball
        n_steps = 50
        lr = 0.8
        ...
        orig_params = {"T": 50, "init_ang": 0.5, "s0": 46.0, "s1": -23.0, "s2": 0.0}
        n_traj = 500
        params = {"s0": 46.0, "s1": -23.0, "s2": 0.0}
        var_name = "ang"
        model = pid
        n_steps = 1000
        lr = 0.2
