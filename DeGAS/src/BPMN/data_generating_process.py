...
def simulate_process_once(
    *,
    ...
    # ACTIVITY 0
    # -------------------------------------------------
    tactivities[0] = sample_positive(lambda: _muinfo * _muinfo + std_normal())
    startacts[0] = t
    ...
    # -------------------------------------------------
    tactivities[2] = sample_positive(lambda: _mupack * _mupack + std_normal())
    tactivities[3] = sample_positive(lambda: _muprepinv * _muprepinv + std_normal())
    tactivities[4] = sample_positive(lambda: _musendinv * _musendinv + std_normal())
    ...
    def schedule(task_id):
        ...
        if R < 1.5:
            ...
            s, e, resload0, resload1 = choose_resource(resload0, resload1, dur)
            startacts[task_id] = s
            endacts[task_id] = e
