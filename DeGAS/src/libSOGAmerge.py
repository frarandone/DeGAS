...
def kmeans_prune(output_dist, Kmax):
    ...
    if output_dist.gm.n_comp() <= Kmax:
        ...
        new_pis.scatter_add_(
            0, labels.view(-1, 1), output_dist.gm.pi
        )  # these are the weights of the new mixture
        normalized_pis = (
            output_dist.gm.pi / new_pis[labels]
        )  # these are the weights normalized for each cluster

        # computes the new means
        weighted_mus = output_dist.gm.mu * normalized_pis
        new_mus = torch.zeros(Kmax, d)
        new_mus.scatter_add_(
            0, labels.view(-1, 1).expand(-1, d), weighted_mus
        )  # these are the means of the new mixture

        # computes the new covariances
        diff = output_dist.gm.mu - new_mus[labels]
        weighted_sigmas = normalized_pis.view(
            -1, 1, 1
        ) * output_dist.gm.sigma + torch.einsum(
            "bi,bj->bij", diff, diff
        ) * normalized_pis.view(-1, 1, 1)
        new_sigmas = torch.zeros(Kmax, d, d)
        new_sigmas.scatter_add_(
            0, labels.view(-1, 1, 1).expand(-1, d, d), weighted_sigmas
        )
...
def merge_comp(current_dist, i, j, tot_mean):
    ...
    tot_cov = (pi_pair.view(-1, 1, 1) * sigma_pair).sum(dim=0) + torch.mm(
        v.t(), pi_pair * v
    )
...
def classic_prune(current_dist, Kmax):
    ...
    if current_dist.gm.n_comp() > Kmax:
        ...
        matrix_mu = compute_matrix_mean(
            current_dist
        )  # elem (i,j) is the weighted sum of mu_i and mu_j
        dist_matrix = torch.sum(
            torch.pow((current_dist.gm.mu - matrix_mu), 2), axis=2
        ).T  # elem (i,j) is the distance between mu_i and the weighted sum of mu_i and mu_j
        weight_dist_matrix = (
            current_dist.gm.pi.view(-1, 1) * dist_matrix
        )  # row i is pi_i * || mu_i - weighted_sum_ij ||^2
        cost_matrix = (
            weight_dist_matrix + weight_dist_matrix.T
        )  # elem (i,j) is the cost of merging i and j

        while n > Kmax:
            # Computes indices of components with minimal cost
            i_indices, j_indices = torch.triu_indices(
                cost_matrix.size(0), cost_matrix.size(1), offset=1
            )
            ...
            if n > Kmax:
                ...
                new_cost = torch.sum(
                    torch.pow(
                        (current_dist.gm.mu[-1].unsqueeze(0) - matrix_mu[:, -1]), 2
                    ),
                    axis=1,
                ).unsqueeze(0)
