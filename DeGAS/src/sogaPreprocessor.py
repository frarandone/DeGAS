    ...
    for idx, m in enumerate(matches):
        ...
        if f"uniform({m_k})" not in computedDist:
            ...
            input_prog = input_prog.replace(
                ...
                "gm([%s],[%s],[%s])"
                % (
                    ",".join(map(lambda x: "%.10f" % (x), weights)),
                    ",".join(map(lambda x: "%.10f" % (x), means)),
            ...
            input_prog = input_prog.replace(
                ...
                "gm([%s],[%s],[%s])"
                % (
                    ",".join(map(lambda x: "%.10f" % (x), w)),
                    ",".join(map(lambda x: "%.10f" % (x), mu)),
...
def compileExpRnd(input_prog):
    ...
    for idx, m in enumerate(matches):
        ...
        if f"exprnd({m_k})" not in computedDist:
            X = np.random.exponential(float(m.split(",")[0].strip()), nsamples).reshape(
                -1, 1
            )
...
def compileGauss(input_prog):
    ...
    for idx, m in enumerate(matches):
        ...
        input_prog = input_prog.replace(oText[idx], "gm([1.0],[%f],[%f])" % (mean, std))
...
def fitGmm(X=None, ncomp=2):
    gmm = GaussianMixture(
        n_components=ncomp, max_iter=1000, n_init=1, covariance_type="full"
    )  # ,init_params="k-means++")
    gmm.fit(X)
...
if __name__ == "__main__":
    ...
    res = re.split(
        r"(?<=\])\s*,", gmEM.replace("gm", "").replace("(", "").replace(")", "")
    )
    w1 = json.loads(res[0])
    ...
    res = re.split(
        r"(?<=\])\s*,", gmFr.replace("gm", "").replace("(", "").replace(")", "")
    )
    w2 = json.loads(res[0])
