...
def get_vars(process_name):
    ...
    if process_name == "healthiness":
        ...
        dependencies = {
            "exercise": ["healthconscius", "littlefreetime"],
            "gooddiet": ["healthconcius"],
            "normalweight": ["gooddiet", "exercise"],
            "colesterol": ["gooddiet"],
            "tested": ["colesterol"],
        }
        weights = {
    ...
    if process_name == "tugwaraddition":
        data_var_list = ["skill1", "skill2", "skill3p1wins"]
...
def get_params(process_name):
    ...
    if process_name == "surveyunbiased":
        ...
        return true_params, init_params, lr, lr_VI, mcmc_steps, mcmc_warmup
    if process_name == "trueskills":
        true_params = {"pa": 105.0, "pb": 90.0, "pc": 120.0}
        init_params = {"pa": 100.0, "pb": 100.0, "pc": 100.0}
        lr = 0.2
        lr_VI = 0.2
        mcmc_steps = 500
        mcmc_warmup = 50
    ...
    if process_name == "test2":
        ...
        return true_params, init_params, lr, lr_VI, mcmc_steps, mcmc_warmup
    if process_name == "pid":
        true_params = {"s0": 46.0, "s1": -23.0, "s2": 0.001}
        init_params = {"s0": 46.0, "s1": -23.0, "s2": 0.001}
        lr = 0.2
        lr_VI = 0.05
        mcmc_steps = 500
        mcmc_warmup = 50
...
def generate_eyecolor_dataset(data_size):
    ...
    for _ in range(data_size):
        ...
        if eyecolor == 0:
            haircolor = np.random.choice(
                [0, 1, 2, 3, 4], p=[0.8, 0.05, 0.04, 0.01, 0.1]
            )
        elif eyecolor == 1:
            haircolor = np.random.choice(
                [0, 1, 2, 3, 4], p=[0.7, 0.15, 0.04, 0.01, 0.1]
            )
        elif eyecolor == 2:
            haircolor = np.random.choice([0, 1, 2, 3, 4], p=[0.4, 0.3, 0.18, 0.02, 0.1])
        else:
            haircolor = np.random.choice(
                [0, 1, 2, 3, 4], p=[0.4, 0.29, 0.18, 0.03, 0.1]
            )
...
def generate_altermu_dataset(data_size, params):
    ...
    for _ in range(data_size):
        w1 = np.random.normal(p1, 5.0)
        w2 = np.random.normal(p2, 5.0)
        w3 = np.random.normal(p3, 5.0)
        mean = w1 * w2
        mean = 3 * mean - w3
        y = np.random.normal(mean, 1.0)
        data.append([y])
    return data
