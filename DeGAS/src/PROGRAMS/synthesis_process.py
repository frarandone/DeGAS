...
def run_optimization(
    ...
    # repeat t
    loss_list, time, number_of_iterations = optimize(
        cfg, params_dict, loss, n_steps=steps, lr=lr, print_progress=False
    )
    ...
    if sensitivity_analysis:
        ...
        avg_error = np.mean(
            [
                abs(params_dict[key].item() - true_value) / abs(true_value)
...
def process(name, all=False):
    ...
    if True:  # all:
        ...
        for lr in [0.001, 0.0005, 0.01, 0.05, 0.1, 0.2]:
            try:
                loss_list, iterations, time_VI = run_inference(
                    model,
                    guide,
                    model_params=(data_size, torch.tensor(data, dtype=torch.float64)),
                    n_steps=1000,
                    lr=lr,
                )
                error = np.mean(
                    [
                        abs(pyro.param(key + "_map").item() - true_value)
                        / abs(true_value)
                        for key, true_value in true_params.items()
                    ]
                )
    ...
    for i in range(10):
        ...
        params_list.append(
            {key: pyro.param(key + "_map").item() for key in true_params.keys()}
        )
    ...
    for params in params_list:
        ...
        error = np.mean(
            [
                abs(params[key] - true_value) / abs(true_value)
                for key, true_value in true_params.items()
            ]
        )
    ...
    np.savetxt(
        f"errors_csv/errors_VI_{program}.csv",
        np.array([mean_error_VI]),
        delimiter=",",
        header="R2_VI",
        comments="",
    )
    ...
    np.savetxt(
        f"losses_csv/losses_VI_{program}.csv",
        np.vstack((mean_loss_VI, std_loss_VI)).T,
        delimiter=",",
        header="mean,std",
        comments="",
    )
    ...
    np.savetxt(
        f"time_csv/time_VI_{program}.csv",
        np.array([np.mean(time_list_VI)]),
        delimiter=",",
        header="time_VI",
        comments="",
    )
    np.savetxt(
        f"iters_csv/iters_VI_{program}.csv",
        np.array([np.mean(iterations)]),
        delimiter=",",
        header="iterations_VI",
        comments="",
    )
    # save the params value in a csv file
    ...
    if all:
        ...
        while rhat > 1.05 and num_samples < 60000:
            ...
            if num_samples > 2000:
                ...
                warmup_steps += 100
            print(
                f"Running MCMC with num_samples={num_samples}, warmup_steps={warmup_steps}"
            )
    ...
    avg_error = np.mean(
        [
            abs(torch.mean(samples[key]) - true_value) / abs(true_value)
            for key, true_value in true_params.items()
        ]
    )
    print(f"Average error: {avg_error}")
    ...
    if name == "all":
        ...
        for program in programs:
            ...
            with open("results_all.csv", "a") as f:
                f.write(
                    f"{program},{time_VI},{error_VI},{time_mcmc},{error_mcmc},{rhat},{neff},{time_degas},{error_degas}\n"
                )
    else:
        process(name)
