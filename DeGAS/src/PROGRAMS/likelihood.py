...
def compute_likelihood(output_dist, data_var_list, data):
    ...
    try:
        data_var_index = [
            output_dist.var_list.index(element) for element in data_var_list
        ]
    except (
        ValueError
    ):  # if the program doesn't have all the variables we are using for the likelihood
    ...
    for k in range(output_dist.gm.n_comp()):
        ...
        if len(mu_not_delta) >= 1:  # if there is at least one continuous variable
            continuous_pdf = (
                output_dist.gm.pi[k]
                * MultivariateNormal(mu_not_delta, sigma_not_delta)
                .log_prob(data[:, not_deltas])
                .exp()
            )
        else:
        ...
        #    raise
        likelihood += (
            continuous_pdf * discrete_pmf
        )  # sums likelihood of every data over all components
