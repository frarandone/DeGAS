...
def run_inference(model, guide, model_params, n_steps=1000, lr=0.05):
    ...
    for step in range(n_steps):
        ...
        if (
            step > 30
            and abs(loss_list[-1] - loss_list[-2]) < 1e-8
            and all(abs(loss_list[-j] - loss_list[-j - 1]) < 1e-8 for j in range(2, 31))
        ):
...
def model_burglary(params):
    ...
    pb = pyro.sample("pb", dist.Beta(1.0, 1.0))
    pe = pyro.sample("pe", dist.Beta(1.0, 1.0))

    with pyro.plate("data", N):
        # --- Burglary ---
        burglary = pyro.sample("burglary", dist.Bernoulli(pb), obs=burglary_obs)

        # --- Earthquake ---
        earthquake = pyro.sample("earthquake", dist.Bernoulli(pe), obs=earthquake_obs)

        # --- Alarm ---
        alarm_prob = torch.where(
            (burglary.bool() | earthquake.bool()), torch.tensor(1.0), torch.tensor(0.0)
        )
        alarm = pyro.sample("alarm", dist.Bernoulli(alarm_prob), obs=alarm_obs)

        # --- Mary wakes ---
        mary_prob = torch.where(
            alarm.bool() & earthquake.bool(),
            torch.tensor(0.8),
        ...
        # --- Phone working ---
        phone_prob = torch.where(
            earthquake.bool(), torch.tensor(0.7), torch.tensor(0.99)
        )
...
def guide_burglary(params):
    ...
    pb_map = pyro.param(
        "pb_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
    pe_map = pyro.param(
        "pe_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
...
def model_clickgraph(params):
    ...
    with pyro.plate("data", N):
        ...
        beta1 = pyro.sample("beta1", dist.Uniform(0.0, 1.0))
        # Conditional definition for beta2
        beta2_same = beta1
        beta2_diff = pyro.sample("beta2_diff", dist.Uniform(0.0, 1.0))
...
def guide_clickgraph(params):
    ...
    p_map = pyro.param(
        "p_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
    pyro.sample("p", dist.Delta(p_map))
...
def model_clinicaltrial(params):
    ...
    pe = pyro.sample("pe", dist.Uniform(0.0, 1.0))  # probability of effect
    pt = pyro.sample("pt", dist.Uniform(0.0, 1.0))  # treatment success prob
    pc = pyro.sample("pc", dist.Uniform(0.0, 1.0))  # baseline control prob
...
def guide_clinicaltrial(params):
    ...
    pt_map = pyro.param(
        "pt_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
    pc_map = pyro.param(
        "pc_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
...
def guide_coinbias(params):
    ...
    p1_map = pyro.param(
        "p1_map", torch.tensor(1.0), constraint=dist.constraints.positive
    )
    p2_map = pyro.param(
        "p2_map", torch.tensor(1.0), constraint=dist.constraints.positive
    )
...
def model_grass(params):
    ...
    pcloudy = pyro.sample("pcloudy", dist.Uniform(0.0, 1.0))
    p1 = pyro.sample("p1", dist.Uniform(0.0, 1.0))
    p2 = pyro.sample("p2", dist.Uniform(0.0, 1.0))
    p3 = pyro.sample("p3", dist.Uniform(0.0, 1.0))
    ...
    with pyro.plate("data", N):
        ...
        sprinkler_prob = torch.where(
            cloudy.bool(), torch.tensor(0.1), torch.tensor(0.5)
        )
        rain = pyro.sample("rain", dist.Bernoulli(rain_prob), obs=data[:, 0])
        sprinkler = pyro.sample(
            "sprinkler", dist.Bernoulli(sprinkler_prob), obs=data[:, 1]
        )
        ...
        # wetRoof logic
        wetRoof = torch.where(
            (temp1 == 1) & (rain == 1), torch.tensor(1.0), torch.tensor(0.0)
        )
        pyro.sample("wetRoof", dist.Bernoulli(wetRoof), obs=data[:, 3])

        # OR nodes and wetGrass
        or1 = torch.where(
            (temp2 == 1) & (rain == 1), torch.tensor(1.0), torch.tensor(0.0)
        )
        or2 = torch.where(
            (temp3 == 1) & (sprinkler == 1), torch.tensor(1.0), torch.tensor(0.0)
        )
        wetGrass = torch.where(
            (or1 == 1) | (or2 == 1), torch.tensor(1.0), torch.tensor(0.0)
        )
        pyro.sample("wetGrass", dist.Bernoulli(wetGrass), obs=data[:, 2])
...
def guide_grass(params):
    ...
    # Variational parameters (unconstrained, so we apply softplus)
    p1_map = pyro.param(
        "p1_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
    p2_map = pyro.param(
        "p2_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
    p3_map = pyro.param(
        "p3_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
    pcloudy_map = pyro.param(
        "pcloudy_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
...
def model_murdermistery(params):
    N, data = params
    palice = pyro.sample("palice", dist.Uniform(0.0, 1.0))
...
def guide_murdermistery(params):
    # Variational parameters (unconstrained, so we apply softplus)
    palice_map = pyro.param(
        "palice_map", torch.tensor(0.9), constraint=dist.constraints.unit_interval
    )
    # Approximate posteriors
...
def guide_noisior(params):
    # Variational parameters (unconstrained, so we apply softplus)
    p0_map = pyro.param(
        "p0_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
    p1_map = pyro.param(
        "p1_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
    ...
    p4_map = pyro.param(
        "p4_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
...
def guide_surveyunbiased(params):
    bias1_map = pyro.param(
        "bias1_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
    bias2_map = pyro.param(
        "bias2_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
...
def model_twocoins(params):
    ...
    with pyro.plate("data", N):
        ...
        both = torch.where(
            (coin1 == 1) & (coin2 == 1), torch.tensor(1.0), torch.tensor(0.0)
        )
        pyro.sample("both", dist.Bernoulli(both), obs=data[:, 0])


def guide_twocoins(params):
    first_map = pyro.param(
        "first_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
    second_map = pyro.param(
        "second_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
...
def guide_altermu2(params):
    ...
    vary_map = pyro.param(
        "vary_map", torch.tensor(1.0), constraint=dist.constraints.positive
    )
...
def guide_normalmixtures(params):
    theta_map = pyro.param(
        "theta_map", torch.tensor(0.5), constraint=dist.constraints.unit_interval
    )
    p1_map = pyro.param("p1_map", torch.tensor(0.0))
    p2_map = pyro.param("p2_map", torch.tensor(0.0))
...
def model_test2(params):
    ...
    with pyro.plate("data", N):
        v = pyro.sample("v", dist.Normal(0.0, sigma1))
        y_mean = torch.where(v < p1, 0.0, 1.0)
        y = pyro.sample("y", dist.Bernoulli(y_mean), obs=data[:, 0])


def guide_test2(params):
    p1_map = pyro.param("p1_map", torch.tensor(0.0))
    sigma1_map = pyro.param(
        "sigma1_map", torch.tensor(1.0), constraint=dist.constraints.positive
    )
    # p3_map = pyro.param("p3_map", torch.tensor(0.))
...
def model_pid(params):
    ...
    noise = dist.Normal(0.0, 0.25)
    noise_ang = dist.Normal(0.0, 0.25)
