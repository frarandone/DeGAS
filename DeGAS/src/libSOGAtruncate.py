...
def and_func(self, dist):
    ...
    new_P, new_mu, new_sigma, indexes = compute_moments(
        dist.gm.mu, dist.gm.sigma, a, b, ineq_idx
    )
    # if the whole distribution has zero prob in the truncation
    if len(indexes) == 0:
        return torch.tensor(0.0), dist
...
def ineq_func(self, dist):
    ...
    # some components might have 0 prob in the truncation
    # indexes contains the indexes of the components that have non-zero probability
    new_P, new_transl_mu, new_transl_sigma, indexes = compute_moments(
        transl_mu, transl_sigma, a, b
    )
    # if the whole distribution has zero prob in the truncation
    if len(indexes) == 0:
        return torch.tensor(0.0), dist
    ...
    new_sigma = torch.matmul(torch.matmul(A_inv, new_transl_sigma), A_inv.t())[
        :, :old_dim, :old_dim
    ]
...
def eq_func(self, dist):
    ...
    cond_sigma = cond_sigma - (1 / dist.gm.sigma[:, obs_idx, obs_idx]).view(
        -1, 1, 1
    ) * torch.bmm(
        dist.gm.sigma[:, select, obs_idx].unsqueeze(2),
        dist.gm.sigma[:, obs_idx, select].unsqueeze(1),
    )
    cond_mu = (
        dist.gm.mu[:, select]
        + (1 / dist.gm.sigma[:, obs_idx, obs_idx]).view(-1, 1)
        * (eq_const - dist.gm.mu[:, obs_idx]).view(dist.gm.sigma.shape[0], 1)
        * dist.gm.sigma[:, select, obs_idx]
    )
    ...
    new_pi = torch.where(all_zeros, 0.0, dist.gm.pi.flatten()).view(-1, 1)
    # normalizes weights
    norm_fact, norm_new_pi = normalize_weights(new_pi)
    ...
    mask[obs_idx] = False
    new_cond_sigma = torch.zeros(
        (cond_sigma.shape[0], cond_sigma.shape[1] + 1, cond_sigma.shape[2] + 1)
    )
    C = new_cond_sigma[:, mask, :]
    C[:, :, mask] = cond_sigma
    new_cond_sigma[:, mask, :] = C
    new_cond_sigma[:, obs_idx, obs_idx] = torch.ones(cond_sigma.shape[0]) * SMOOTH_EPS
    return norm_fact, Dist(
        dist.var_list, GaussianMix(norm_new_pi, new_cond_mu, new_cond_sigma)
    )
...
class TruncRule(TRUNCListener):
    ...
    def __init__(self, var_list, data, params_dict):
        ...
        self.const = torch.tensor(0.0)  # constant r.h.s. of the inequality
    ...
    def enterAnd_trunc(self, ctx):
        ...
        self.coeff[idx] = torch.tensor(1.0)
        inops = ctx.inop()
    ...
    def enterLexpr(self, ctx):
        self.flag_sign = torch.tensor(1.0)
        monom = ctx.monom()[0]
...
def compute_moments(mu, sigma, a, b, idx=0):
    ...
    if trunc:
        ...
        new_sigma = compute_mom2(
            mu[indexes], sigma[indexes], a, b, trunc, new_P[indexes], new_mu, muj
        )
    else:  # and_func
        new_sigma = compute_mom2_and(
            mu[indexes], sigma[indexes], a, b, new_P[indexes], new_mu, idx
        )
    return new_P[indexes], new_mu, new_sigma, indexes
...
def compute_lower_mom(mu, sigma, a, b, trunc):
    ...
    if trunc == "low":
        muj = (
            mu[:, 1:]
            + ((a[0] - mu[:, 0]) / sigma[:, 0, 0]).view(-1, 1) * sigma[:, 1:][:, :, 0]
        )
    elif trunc == "up":
        muj = (
            mu[:, 1:]
            + ((b[0] - mu[:, 0]) / sigma[:, 0, 0]).view(-1, 1) * sigma[:, 1:][:, :, 0]
        )
    return muj
...
def compute_mom2(mu, sigma, a, b, trunc, new_P, new_mu, muj):
    ...
    if trunc == "low":
        C[:, :, 0] += (
            norm.log_prob(a[0]).exp().view(-1, 1)
            * (a[0] ** e0)
            * torch.hstack((torch.ones((c, 1)), muj))
        )
    elif trunc == "up":
        C[:, :, 0] += (
            -norm.log_prob(b[0]).exp().view(-1, 1)
            * (b[0] ** e0)
            * torch.hstack((torch.ones((c, 1)), muj))
        )
    # computes the new matrix
    new_sigma = new_P.view(c, 1, 1) * torch.matmul(
        mu.unsqueeze(2), new_mu.unsqueeze(1)
    ) + torch.matmul(sigma, C.transpose(1, 2))
    new_sigma = new_sigma / new_P.view(c, 1, 1) - torch.matmul(
        new_mu.unsqueeze(2), new_mu.unsqueeze(1)
    )
    return new_sigma
...
def compute_mom2_and(mu, sigma, a, b, new_P, new_mu, idx):
    ...
    C = (new_P.view(c, 1, 1)) * torch.eye(n).unsqueeze(0).expand(c, -1, -1)
    norm = distributions.Normal(loc=mu[:, idx], scale=torch.sqrt(sigma[:, idx, idx]))
    C[:, :, idx] += norm.log_prob(a[idx]).exp().view(-1, 1) * (
        a[idx] ** e0
    ) * compute_muj(a, mu, sigma, idx) - norm.log_prob(b[idx]).exp().view(-1, 1) * (
        b[idx] ** e0
    ) * compute_muj(b, mu, sigma, idx)
    # computes the new matrix
    new_sigma = new_P.view(c, 1, 1) * torch.matmul(
        mu.unsqueeze(2), new_mu.unsqueeze(1)
    ) + torch.matmul(sigma, C.transpose(1, 2))
