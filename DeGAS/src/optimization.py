...
def optimize(cfg, params_dict, loss_func, n_steps=100, lr=0.05, print_progress=True):
    ...
    optimizer = torch.optim.Adam(
        [params_dict[key] for key in params_dict.keys()], lr=lr
    )
    ...
    for i in range(n_steps):
        ...
        # loss
        current_dist = start_SOGA(
            cfg, params_dict
        )  # we compute the output distribution for the current values of the parameters using start_SOGA
        loss = loss_func(
            current_dist
        )  # we compute the loss using the set of trajectories and the current output distribution
