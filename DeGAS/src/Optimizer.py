...
thermo_params = {
    "model": thermostat,  # name of the model to sample from in models.py
    "true_params": {
        "T": 30,
        "init_T": 16.0,
        "tOn": 17.0,
    ...
    "pars": {"tOff": 22.0, "tOn": 15.0},  # initial guess for the parameters to optimize
    "loss": lambda dist: neg_log_likelihood(
        traj_set, dist, idx=list(range(1, 30))
    ),  # loss function
}
...
pid_params = {
    ...
    "pars": {"s0": 46.0, "s1": -23.0, "s2": 0.0},
    "loss": lambda dist: signal_error(dist),
}

model_list = {
    "thermostat": thermo_params,
...
if __name__ == "__main__":
    for model_name, model in model_list.items():
        ...
        if n_traj:
            traj_set = generate_trajectories(
                orig_model, n_traj, model_params=true_params
            )
