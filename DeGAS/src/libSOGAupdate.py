...
class AsgmtRule(ASGMTListener):
    ...
    def enterAdd(self, ctx):
        ...
        if self.is_prod:
            ...
            self.add_const = torch.tensor(0.0)
        ...
        if self.is_prod:
            for term in ctx.term():
                if term.gm() is not None:
                    ...
                    self.mul_idx.append(
                        self.var_list.index(term.symvars().getVar(self.data))
                    )
    ...
    def exitAdd(self, ctx):
        if not self.is_prod:
            ...
            # this part makes the distribution non differentiable but is needed for the smoother
                ...
                self.func = partial(
                    const_func, self
                )  # check if this is needed after the new smoother is ready
...
def add_func(self, dist):
    ...
    extended_sigma[:, i, :] = extended_sigma[:, :, i] = torch.matmul(
        self.add_coeff, extended_gm.sigma
    )
    extended_sigma[:, i, i] = torch.matmul(
        torch.matmul(self.add_coeff, extended_gm.sigma), self.add_coeff.reshape(-1, 1)
    ).flatten()
    new_dist = Dist(
        dist.var_list,
        GaussianMix(
...
def mul_func(self, dist):
    ...
    extended_mu = torch.clone(extended_gm.mu)
    extended_mu[:, i] = (
        extended_gm.sigma[:, j, k] + extended_gm.mu[:, j] * extended_gm.mu[:, k]
    )
    extended_sigma = torch.clone(extended_gm.sigma)
