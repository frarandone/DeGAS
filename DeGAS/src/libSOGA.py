...
def start_SOGA(
    cfg, params_dict={}, pruning="classic", Kmax=None, parallel=None, useR=False
):
...
def SOGA(node, data, parallel, pruning, exec_queue, params_dict):
    ...
    if node.type == "state":
        ...
        if current_p > TOL_PROB:
            current_dist = update_rule(
                current_dist, expr, data, params_dict
            )  ### see libSOGAupdate
        ...
        if child.type == "loop" and data[child.idx][0] is not None:
            data[child.idx][0] += 1
    ...
    if node.type == "prune":
        current_dist = prune(
            current_dist, pruning, node.Kmax
        )  ### options: 'classic', 'ranking' (see libSOGAmerge)
        node.list_dist = []
        child = node.children[0]
        update_child(child, current_dist, current_p, current_trunc, exec_queue)
